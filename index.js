...
=> NOTES
• • WANT TO TAKE A CASE? ADD NAME CHIRAG YOUTUBER IN THX TO
• DO NOT SELL
• THX TO DON'T CHANGE
• NAME OTHER CHIRAG YOUTUBER DO NOT DELETE
*/
const {
    WAConnection,
    MessageType,
...
audio20 = fs.readFileSync('./X3/nayla20.m4a') 
		...
		//	if (nay.key.fromMe) return
            ...
            ranm = getRandom('.mp3')
            gtts3.length > 600
            ? reply('The text is too much omg+')
			...
			const timers = (teks) => {
			setTimeout( () => {reply("[❗] 20 MORE SECONDS")}, 10000)
			setTimeout( () => {reply("[❗] 10 MORE SECONDSI")}, 20000)
			setTimeout( () => {reply("[❗] TIME HAS RUN OUT" + `\n${tz} *JAWABAN* : ${teks}`)}, 30000)}	
            ...
            // REPLY ONLY  
            only = {
            botadmin : "SORRY THIS FEATURE CAN ONLY BE USED IF THE BOTS BECOME AN ADMIN",           
            group : "SORRY THIS FEATURE CAN ONLY BE USED IN GROUP",           
            admin : "SORRY THIS FEATURE CAN ONLY BE USED BY GROUP ADMINS",
            owner : "SORRY THIS FEATURE CAN ONLY BE USED BY THE OWNER BOT",
            member : "SORRY THIS FEATURE CAN ONLY BE USED BY GROUP MEMBERS",
            pribadi : "SORRY THIS FEATURE CAN ONLY BE USED ON PRIVATE ORDER",
            premium : "SORRY THIS FEATURE CAN ONLY BE USED PREMIUM USERS"
            }  
            ...
            const B1 = {contentText: "DO YOU LIKE THIS BOT?", footerText: `Berikan pendapat anda...`,buttons: A1,headerType: 1}      
			nayla.sendMessage(from, B1, MessageType.buttonsMessage, {quoted:nay1})
			}
			if (buttonsR === 'NO') {	
			reply("THANK YOU FOR GIVING YOUR OPINION ABOUT THIS BOTT")
			nayla.sendMessage(`${oownerNumber}@s.whatsapp.net`, `[ *NEW MESSAGE* ]\n${tz} *NAME* : ${codename}\n${tz} *TIME* : ${time}\n${tz} *PENDAPAT* : TIDAK MENYUKAI BOT INI`, text,{quoted:nay1})
			}
			if (buttonsR === 'YES') {	
			reply("THANK YOU FOR GIVING YOUR OPINION ABOUT THIS BOT")
            ...
            reply("THAT'S MY OWNER'S NUMBER")
            }
			...
			try{
	        checkLimit(sender)
	        } catch (e) { reply("PLEASE USE 1 FEATURE FIRST")}
		    break 
		    case 'claim':
		    reply("SUCCESS!, YOU GET 25 LIMIT FOR FREE, LIMIT WILL BE INSTALLED AFTER 60 SECONDS, PLEASE WAIT A LOT MORE" + `\n*NOTE* : JANGAN SPAM FITUR INI`)
		    setTimeout( () => {
		    bayarLimit(sender, "25")
		    nayla.sendMessage(`${sender.split("@")[0]}@s.whatsapp.net`, "LIMIT 15 YOU'RE LOGGED IN, YOU CAN USE THE FEATURES ON THIS BOTT BACK", text, {quoted:nay1})
		    }, 60000)
		    break
			case 'addprem':
			if (isLimit(sender)) return
			...
			if (!isGroupAdmins) return reply(only.admin)
			if (!isBotGroupAdmins) return reply(only.botadmin)				 
			if (args.length < 1) return reply('WHERES THE NUMBER')
			...
			console.log('Error :', e)
			reply('FAILED ADDING TARGET, MAYBE BECAUSE IT WAS PRIVATE')
			}
			break			 		 
			case 'listonline':
			if (isLimit(sender)) return
            ...
            nayla.sendMessage(from, 'SUCCES, CHANGE THE GROUP NAME', text, {quoted: nay1 })			
			break
            case 'setdesc':
			if (isLimit(sender)) return
            await limitAdd(sender)
            ...
            nayla.sendMessage(from, 'SUCCES, CHANGE GROUP DESCRIPTION', text, {quoted:nay1 })				
			break	
	    	case 'listadmin':
			if (isLimit(sender)) return
            await limitAdd(sender)
            ...
            if (isBanned) return reply("MAAF ANDA TELAH DI BANNED") 			 
         	...
         	.on('end', function () {
         	console.log('DONE DONT FORGET TO SUBSCRIBE YT CHIRAG YOUTUBER')
			...
			if (isLimit(sender)) return
            await limitAdd(sender)
            if (isBanned) return reply("SORRY YOU HAVE BEEN BANNED") 							
			reply(prosess)
			try{			
